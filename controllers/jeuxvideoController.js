...
db.run("INSERT INTO Jeux_videos(titre) VALUES (?)",[/*jeu_id,*/titre],
    ...
    }res.json({message:"Jeu ajouté",jeu_id:this.lastID});});
};
...
db.run('UPDATE Jeux_videos SET titre=? WHERE jeu_id=?',[/*jeu_id, */titre, id],
    function(err){
        if(err){
            return res.status(500).json({ erreur: err.message });
        }
        res.json({message: "Jeu vidéo modifié",jeu_id: id});
    });
};
